...
def authentication(login_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
...
def update_profile_image(user: User = Depends(get_current_user),
    image: UploadFile = File(None),
    db: Session = Depends(get_db)):
...
def update_password(user: User = Depends(get_current_user), 
    password: str = Form(),
    db: Session = Depends(get_db)):
